...
(async () => {
  ...
  await withPage(async (page) => {
    ...
    const det = await page.evaluate(() => ({
      ...
      shot: (() => { const i = document.querySelector('.d-shot img'); return !!i && i.naturalWidth > 100; })(),
      minis: document.querySelectorAll('.mini').length,
    }));
    await page.screenshot({ path: path.join(SHOTS, '18-detail.png') });
    report('tool-detail-page', det.title && det.about > 200 && det.feats >= 4 && det.steps >= 3 && det.verify && det.shot && det.minis >= 15,
      `detail.html?id=exif: ${det.about}-char story, ${det.feats} features, ${det.steps} steps, verification + screenshot + ${det.minis} library cards`);
    // unknown id shows the friendly catalog, not a dead end
