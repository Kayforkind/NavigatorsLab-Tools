...
# Ongoing updates go through funnel-sync.py; funnel-gen.py must run first.
...
for slug, name in REPO.items():
    ...
    home = f"https://navigatorslab.com/{t['id']}"  # pretty URL (edge worker 301s)
    ...
    run(['git', '-c', 'user.name=Kazim', '-c', 'user.email=kazim.r.merchant@gmail.com',
         'commit', '-m',
         'NavigatorsLab - ' + t['name'] + ': full product repo — the real tool runs on this repo’s GitHub Pages (demo.html) and on navigatorslab.com\n'
         '\nFree, open-source (MIT). Landing + in-depth README + runnable demo, generated by scripts/funnel-gen.py.\n'
